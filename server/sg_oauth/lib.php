...
function sg_oauth_lookup_consumer_entity($consumer_key) {
    global $DB;
...
function sg_oauth_consumer_from_entity($consumEnt) {
...
function sg_oauth_find_parameters($url) {
    $bits = parse_url($url);
...
function sg_oauth_lookup_nonce_entity($consumer, $token, $nonce) {
    global $DB;
    if (!$consumer || !$nonce) {
        return NULL;
    }
...
function sg_oauth_save_nonce($consumerKey, $nonce, $tokenKey = NULL, $timeout = 600) {
    global $DB;
...
function sg_oauth_save_request_token($token, $consumer, $user = NULL, $callback = NULL) {
    global $DB;
...
function sg_oauth_save_validated_token($token) {
    global $USER, $DB;
    ...
    $tokEnt->validated = 1;
    $verifier = sg_oauth_generate_verifier();
...
function sg_oauth_save_access_token($tokEnt, $token) {
    global $DB;
    $accEnt = new stdClass();
    $accEnt->id = $tokEnt->id;
...
function sg_oauth_lookup_token_entity($tokenKey, $token_type, $consumer = false) {
    global $DB;
...
function sg_oauth_token_from_entity($tokEnt) {
    return new OAuthToken($tokEnt->token, $tokEnt->secret);
}

// generate a verifier code
function sg_oauth_generate_verifier($length = 8) {
...
function sg_oauth_lookup_access_token_entity($consumerkey, $userid) {
    global $DB;
...
// execute a rest call
function sg_oauth_execute_rest($consumer, $token, $service = '') {
    global $DB;
    if ($tokEnt = sg_oauth_lookup_token_entity($token->key, 'access', $consumer)) {
        if ($tokEnt->userid > 0) {
            require_once 'services.php';
            // For compatibity with old services names including symbol . in their name we convert '.' to '_'
            // This is needed because we can't name functions with '.' symbol in their names
            $service = str_replace('.', '_', $service);
            // Service should be a function with this name in services.php
            $function_name = "sg_oauth_service_$service";
            if (function_exists($function_name)) {
                return $function_name($consumer, $tokEnt);
            } else {
                return NULL;
            }
        } else {
...
function sg_oauth_delete_token_entity($token) {
    global $DB;
    $DB->delete_records('oauth_tokens', array('id' => $token->id));
}

function sg_oauth_refresh_token_lastcheckedon($token) {
    global $DB;
...
function sg_oauth_user_clear_lastchecked($user) {
    global $DB;
...
// cleanup data
function sg_oauth_cleanup_tokens_and_nonces() {
    global $DB;
    ...
    if ($tokens = $DB->get_records('oauth_tokens')) {
        foreach ($tokens as $token) {
            $consumEnt = sg_oauth_lookup_consumer_entity($token->consumerkey);
            if (!$consumEnt) {
